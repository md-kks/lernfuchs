...
class DialogueDefinitionLoader {
  const DialogueDefinitionLoader({AssetBundle? assetBundle})
    : _assetBundle = assetBundle;

  final AssetBundle? _assetBundle;

  Future<DialogueLibrary> loadFromAsset(String assetPath) async {
    final bundle = _assetBundle ?? rootBundle;
    final raw = await bundle.loadString(assetPath);
    return DialogueLibrary.fromJson(
