...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  Future<void> _load() async {
    ...
    final expedition = await ExpeditionGenerator().shouldTriggerExpedition('fluesterwald');
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          return Stack(
            children: [
              CustomPaint(
                painter: _HomeForestPainter(t: _controller.value, playedToday: _playedToday),
                ...
                child: Stack(
                  children: [
                    ...
                    Align(
                      ...
                      child: _HomeButton(
                        text: _gameCompleted ? 'Erinnerungen →' : 'Flüsterwald erkunden →',
                    ...
                    Positioned(
                      ...
                      child: _BaumhausButton(
                        onTap: () => Navigator.push(
                          context,
                          MaterialPageRoute(builder: (_) => const BaumhausScreen()),
                        ),
                      ),
                    ),
                  ],
...
class _HomeForestPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(Rect.fromLTWH(0, size.height * 0.80, size.width, size.height * 0.20), Paint()..color = const Color(0xFF1E1200));
    final center = playedToday ? Offset(size.width - 58 * sc, 138 * sc) : Offset(size.width / 2, size.height * 0.61);
    final scale = playedToday ? sc * 0.8 : sc * 1.5;
    final flap = math.sin(t * math.pi * 8) * 5 * scale;
    canvas.drawCircle(center, 16 * scale, Paint()..color = const Color(0xFFFF8F00));
    canvas.drawPath(Path()..moveTo(center.dx - 9 * scale, center.dy - 11 * scale)..lineTo(center.dx - 18 * scale, center.dy - 25 * scale - flap)..lineTo(center.dx - 2 * scale, center.dy - 16 * scale)..close(), Paint()..color = const Color(0xFF5D4037));
    canvas.drawPath(Path()..moveTo(center.dx + 9 * scale, center.dy - 11 * scale)..lineTo(center.dx + 18 * scale, center.dy - 25 * scale + flap)..lineTo(center.dx + 2 * scale, center.dy - 16 * scale)..close(), Paint()..color = const Color(0xFF5D4037));
  }

  @override
  bool shouldRepaint(covariant _HomeForestPainter oldDelegate) => oldDelegate.t != t || oldDelegate.playedToday != playedToday;
}
...
class _HomePanel extends StatelessWidget {
  ...
  const _HomePanel({required this.text, required this.streak, required this.onDaily});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          if (streak >= 2) Align(alignment: Alignment.centerRight, child: Text('🔥 $streak Tage', style: const TextStyle(color: Color(0xFFFF8F00), fontWeight: FontWeight.bold))),
          Text(text, textAlign: TextAlign.center, style: const TextStyle(color: Color(0xFFE8D5B0), fontSize: 16, fontWeight: FontWeight.w600)),
...
class _DoneBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: const Color(0xFF3E2108), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFF4E8038))),
      child: const Text('Heute erledigt ✓', style: TextStyle(color: Color(0xFFE8F5E9), fontWeight: FontWeight.bold)),
    );
  }
}
